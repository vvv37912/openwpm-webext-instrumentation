...
export class JavascriptInstrument {
  ...
  public listen() {
    this.onMessageListener = (message, sender) => {
      ...
      if (message.namespace && message.namespace === "javascript-instrumentation") {
  ...
  /**
   * Sets the crawl ID, marks the JS instrumentation as configured and
   * sends any pending records that have been received up until this point.
   * @param crawlID
   */
  public run(crawlID) {
